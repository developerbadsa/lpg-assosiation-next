...
async function listStationOwners(): Promise<StationOwnerOption[]> {
  ...
  if (!res.ok) throw new Error(raw?.message ?? 'Failed to load station owners');
  ...
  return rows.map(row => {
    ...
    const phone = str(pick(row?.phone_number, user?.phone_number, row?.phone), '');
...
function mapStationDetailsToForm(data: any): FormState {
  ...
  return {
    ...
    other_businesses: mapOtherBusinesses(
      pick(data?.other_businesses, data?.otherBusinesses, data?.other_business_ids)
    ),
...
export default function StationForm({
  ...
  const handleSubmit = () => {
    ...
    if (!form.station_owner_id || !isValidOptionId(ownerOptions, form.station_owner_id)) {
    ...
    if (!form.division_id || !divisionOptions.length || !isValidOptionId(divisionOptions, form.division_id)) {
      setValidationError('Please select a valid division from the list.');
      return;
    }

    if (!form.district_id || !districtOptions.length || !isValidOptionId(districtOptions, form.district_id)) {
      setValidationError('Please select a valid district from the list.');
      return;
    }

    if (!form.upazila_id || !upazilaOptions.length || !isValidOptionId(upazilaOptions, form.upazila_id)) {
    ...
    if (form.verification_status === 'REJECTED' && !form.rejection_reason.trim()) {
    ...
    if (form.explosive_license && form.explosive_license.size > maxFileSizeBytes) {
    ...
    const payload: StationUpsertPayload = {
      ...
      other_businesses: form.other_businesses
        ...
        .filter(Boolean),
      verification_status:
        statusDefaults.verification_status ?? (form.verification_status || null),
    ...
    <div className='space-y-4'>
      {loadingDetails ? (
        ...
        <div className='space-y-4'>
          <div className='grid gap-4 md:grid-cols-2'>
            ...
            <div>
              ...
              <input
                ...
                onChange={e => {
                  ...
                  const match = ownerOptions.find(o => o.label === value || o.id === value);
                  setForm(prev => ({
                    ...prev,
                ...
                placeholder={loadingOptions ? 'Loading owners...' : 'Select owner'}
                className='h-9 w-full rounded-[6px] border border-black/10 px-3 text-[12px] outline-none focus:border-black/20 disabled:bg-black/5'
              />
            ...
            <div>
              ...
              <DatalistInput
                ...
                disabled={isView}
                placeholder={divisionsQ.isLoading ? 'Loading divisions...' : 'Select division'}
            ...
            <div>
              ...
              <DatalistInput
                ...
                disabled={isView}
                placeholder={districtsQ.isLoading ? 'Loading districts...' : 'Select district'}
            ...
            <div>
              ...
              <DatalistInput
                ...
                disabled={isView}
                placeholder={upazilasQ.isLoading ? 'Loading upazilas...' : 'Select upazila'}
                        ...
                        onChange={() =>
                          setForm(prev => ({
                            ...
                            other_businesses: checked
                              ? prev.other_businesses.filter(id => id !== b.id)
            ...
            <div>
              ...
              <div
                ...
                <span className='flex-1 truncate text-[12px] text-[#2B3A4A]'>
                  {form.explosive_license?.name ?? 'Choose file to upload'}
                </span>
          ...
          {validationError ? (
            <p className='text-[12px] font-medium text-red-600'>{validationError}</p>
          ) : null}
