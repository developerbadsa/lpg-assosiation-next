  ...
  if (statsQ.isError) {
    return (
      <div className="text-sm text-red-600">
        {(statsQ.error as Error)?.message ?? 'Failed to load dashboard data.'}
      </div>
    );
  }
          ...
          <div className="rounded-2xl bg-[#F3F6FF] p-6 shadow-[0_18px_38px_rgba(2,6,23,0.12)]">
            <div className="grid h-[240px] place-items-center">
              <div className="w-[200px] rounded-2xl border-2 border-dashed border-emerald-300/80 bg-white/40 px-6 py-10 text-center">
                ...
                <p className="mt-1 text-[10px] text-slate-500/80">JPG, PNG up to 2MB</p>
              </div>
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="grid h-9 w-9 place-items-center rounded-lg bg-white/15">{icon}</div>
      </div>
    </div>
  );
}
