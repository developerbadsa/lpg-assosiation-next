...
import { useCreateCommitteeMember } from './queries';
...
export default function AddCommitteeModal({
  ...
  const createM = useCreateCommitteeMember();
  ...
  useEffect(() => {
    ...
    // Reset state when modal opens (fresh form every time).
    setPositionName('');
    setPositionSlug('');
  ...
  const fileTooLarge = Boolean(profileImage && profileImage.size > 10 * 1024 * 1024);
      ...
      >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                // Auto-generate slug until the user edits slug manually.
        ...
        {createM.isError ? (
          <div className="mt-4 rounded-[8px] border border-red-200 bg-red-50 p-3 text-[12px] text-red-700">
            {(createM.error as Error)?.message ?? 'Failed to create committee member.'}
          </div>
        ) : null}
