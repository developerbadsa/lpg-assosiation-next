...
function extractId(req: Request, params?: Record<string, any>) {
  ...
  if (!Number.isFinite(id)) throw new LaravelHttpError(400, 'Invalid video id');
  return id;
}
...
export async function POST(req: Request, ctx: { params?: Record<string, any> }) {
  try {
    ...
    // Laravel update convention for file upload
    const data = await laravelFetch(`/videos/${id}?_method=PUT`, {
      method: 'POST',
      auth: true,
      body: formData,
    });
...
export async function DELETE(req: Request, ctx: { params?: Record<string, any> }) {
  try {
    ...
    const data = await laravelFetch(`/videos/${id}`, {
      method: 'DELETE',
      auth: true,
    });
